...
def Add():
    ...
    try:
        ...
        val = (studid, studname, coursename, bouh, price, idcat)
...
def update():
    ...
    try:
        ...
        val = (studname, coursename, bouh, price, idcat, studid)
        mycursor.execute(sql, val)
        mysqldb.commit()
        lastid = mycursor.lastrowid
...
root.title("Gestionnaire base de données de Ama")
...
tk.Label(root, text="Veuillez entrer les informations dans les champs à gauche", fg= "black", font=(None, 30)).place(x=350, y=5)
tk.Label(root, text="ID Catégorie : Fruits = 1, Légume = 2, Epice = 3", font=(None, 30)).place(x=350, y=420)
